...
const GaushalaManagementPage = () => {
  ...
  useEffect(() => {
    ...
    api.get("/temples?limit=1000") // Fetch organizations
      .then((res) => {
        ...
        // Filter to only those organizations that are BHOJANSHALAs or have hasGaushala=true
        orgs = orgs.filter(o => o.type === "BHOJANSHALA" || o.hasGaushala);
  ...
  const handleTogglePublish = async () => {
    ...
    try {
      const payload = { gaushalaPublished: !selectedOrg.gaushalaPublished };
      await api.patch(`/temples/${selectedOrg.id}`, payload);
      toast.success(payload.gaushalaPublished ? "Gaushala published successfully!" : "Gaushala unpublished.");
      setOrganizations(orgs => orgs.map(o => o.id === selectedOrg.id ? { ...o, gaushalaPublished: payload.gaushalaPublished } : o));
    } catch (error) {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Gaushala Management</h1>
        ...
        {(isGlobalScope || organizations.length > 0) && (
          <div className="flex items-center gap-4">
            ...
            >
              <option value="" disabled>-- Select Gaushala --</option>
              {organizations.map((org) => (
                <option key={org.id} value={org.id}>
                  {org.gaushalaName || org.name}
                </option>
              ))}
            </select>
          </div>
        ...
        <div className="p-6 bg-slate-50/50 min-h-[500px]">
          {!selectedOrgId ? (
            <div className="flex items-center justify-center h-64 text-slate-500">
              Please select an organization to manage Gaushala.
            </div>
          ) : (
            <>
...
export default GaushalaManagementPage;

// --- Tab Components ---

const TimingsTab = ({ orgId, selectedOrg }) => {
  const [formData, setFormData] = useState({
    gaushalaBreakfastTiming: "",
    gaushalaBreakfastCharge: "",
    gaushalaLunchTiming: "",
    gaushalaLunchCharge: "",
    gaushalaDinnerTiming: "",
    gaushalaDinnerCharge: "",
  });
  ...
  useEffect(() => {
    ...
    if (orgId) {
      api.get(`/temples/${orgId}`).then(res => {
        ...
        setFormData({
          gaushalaBreakfastTiming: org.gaushalaBreakfastTiming || "",
          gaushalaBreakfastCharge: org.gaushalaBreakfastCharge || "",
          gaushalaLunchTiming: org.gaushalaLunchTiming || "",
          gaushalaLunchCharge: org.gaushalaLunchCharge || "",
          gaushalaDinnerTiming: org.gaushalaDinnerTiming || "",
          gaushalaDinnerCharge: org.gaushalaDinnerCharge || "",
        });
      }).catch(console.error);
    }
  }, [orgId, selectedOrg]);
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input name="gaushalaBreakfastTiming" value={formData.gaushalaBreakfastTiming} onChange={handleChange} type="text" placeholder="08:00 AM - 10:00 AM" className="w-full mt-1 p-2 border rounded-lg text-sm focus:ring-2 focus:ring-orange-500 outline-none" />
              </div>
              <div>
                <label className="text-xs text-slate-500 font-medium">Charge (₹)</label>
                <input name="gaushalaBreakfastCharge" value={formData.gaushalaBreakfastCharge} onChange={handleChange} type="number" placeholder="50" className="w-full mt-1 p-2 border rounded-lg text-sm focus:ring-2 focus:ring-orange-500 outline-none" />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input name="gaushalaLunchTiming" value={formData.gaushalaLunchTiming} onChange={handleChange} type="text" placeholder="12:00 PM - 02:00 PM" className="w-full mt-1 p-2 border rounded-lg text-sm focus:ring-2 focus:ring-orange-500 outline-none" />
              </div>
              <div>
                <label className="text-xs text-slate-500 font-medium">Charge (₹)</label>
                <input name="gaushalaLunchCharge" value={formData.gaushalaLunchCharge} onChange={handleChange} type="number" placeholder="100" className="w-full mt-1 p-2 border rounded-lg text-sm focus:ring-2 focus:ring-orange-500 outline-none" />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input name="gaushalaDinnerTiming" value={formData.gaushalaDinnerTiming} onChange={handleChange} type="text" placeholder="05:30 PM - 07:30 PM" className="w-full mt-1 p-2 border rounded-lg text-sm focus:ring-2 focus:ring-orange-500 outline-none" />
              </div>
              <div>
                <label className="text-xs text-slate-500 font-medium">Charge (₹)</label>
                <input name="gaushalaDinnerCharge" value={formData.gaushalaDinnerCharge} onChange={handleChange} type="number" placeholder="100" className="w-full mt-1 p-2 border rounded-lg text-sm focus:ring-2 focus:ring-orange-500 outline-none" />
              </div>
            </div>
          </div>
        </div>
...
const MenuTab = ({ orgId }) => {
  ...
  const fetchMenu = async () => {
    try {
      setLoading(true);
      const res = await api.get(`/gaushala/${orgId}/menu`, { params: { dayOfWeek: selectedDay } });
  ...
  const handleSaveMenu = async (e) => {
    ...
    try {
      ...
      if (editingItem) {
        await api.put(`/gaushala/${orgId}/menu/${editingItem.id}`, payload);
      } else {
        await api.post(`/gaushala/${orgId}/menu`, payload);
      }
...
const ManagersTab = ({ orgId }) => {
  ...
  const fetchManagers = () => {
    if (!orgId) return;
    setLoading(true);
    api.get(`/gaushala/${orgId}/managers`)
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-6">
        <h3 className="text-lg font-semibold text-slate-800 border-b pb-4">Gaushala Managers</h3>
...
const SettingsTab = ({ orgId, selectedOrg, setOrganizations }) => {
  ...
  useEffect(() => {
    if (selectedOrg) {
      setGaushalaName(selectedOrg.gaushalaName || "");
    }
  }, [selectedOrg]);
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-6">
        <h3 className="text-lg font-semibold text-slate-800 border-b pb-4">Gaushala Settings</h3>
        
        <div className="space-y-6">
          <div className="flex items-center justify-between p-4 bg-slate-50 border border-slate-100 rounded-lg">
            <div>
              <h4 className="font-medium text-slate-800">Publish Gaushala</h4>
              <p className="text-sm text-slate-500 mt-1">Make this Gaushala visible to members on the mobile app.</p>
            </div>
            <button
              onClick={handleTogglePublish}
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${selectedOrg?.gaushalaPublished ? 'bg-green-500' : 'bg-slate-300'}`}
            >
              <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${selectedOrg?.gaushalaPublished ? 'translate-x-6' : 'translate-x-1'}`} />
            </button>
          </div>

          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Gaushala Name (Optional)</label>
            <p className="text-xs text-slate-500 mb-3">If left blank, the temple or organization name will be used.</p>
            <input 
              type="text" 
              placeholder="e.g. Shri Mahavir Gaushala" 
              className="w-full p-2.5 border rounded-lg outline-none focus:ring-2 focus:ring-orange-500 text-sm max-w-md" 
              value={gaushalaName} 
              onChange={(e) => setGaushalaName(e.target.value)} 
            />
          </div>
