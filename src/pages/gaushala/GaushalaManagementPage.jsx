...
export default function GaushalaManagementPage() {
  const { t } = useLanguage();
  ...
  useEffect(() => {
    ...
    api.get("/gaushalas?limit=1000") // Fetch organizations
      .then((res) => {
        ...
        // Filter to only those organizations that are GAUSHALAs or have hasGaushala=true
        orgs = orgs.filter(o => o.type === "GAUSHALA" || o.hasGaushala);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">{t("Gaushala Management")}</h1>
        ...
        {(isGlobalScope || organizations.length > 0) && (
          <div className="flex items-center gap-4">
            ...
            <label className="text-sm font-medium text-slate-700">{t("Select Gaushala")}:</label>
            <select
              value={selectedOrgId || ""}
            ...
            >
              <option value="" disabled>-- {t("Select Gaushala")} --</option>
        ...
        <div className="p-6 bg-slate-50/50 min-h-[500px]">
          {!selectedOrgId ? (
            <div className="flex items-center justify-center h-64 text-slate-500">
              {t("Please select a Gaushala to manage.")}
            </div>
          ) : (
            <>
...
const GalleryTab = ({ orgId }) => {
  ...
  const loadGallery = () => {
    api.get(`/gaushalas/${orgId}`).then(res => {
  ...
  const handleGalleryUpload = async (e) => {
    ...
    setSaving(true);
    try {
      await api.post(`/gaushalas/${orgId}/gallery/bulk`, formData, {
  ...
  const handleDeleteImage = async (imageId) => {
    ...
    setSaving(true);
    try {
      await api.delete(`/gaushalas/${orgId}/gallery/${imageId}`);
  ...
  return (
    <div className="border rounded-lg bg-card overflow-hidden">
      <div className="bg-muted/30 px-6 py-4 border-b flex justify-between items-center">
        <div>
          <h3 className="text-lg font-medium">{t("Gaushala Gallery")}</h3>
      ...
      <div className="p-6">
        {(!gallery || gallery.length === 0) ? (
          <EmptyState 
            ...
            description={t("Upload images to show in your Gaushala profile.")}
          />
        ) : (
...
const SettingsTab = ({ orgId, selectedOrg, setOrganizations }) => {
  ...
  const handleTogglePublish = async () => {
    ...
    try {
      ...
      toast.success(payload.gaushalaPublished ? t("Gaushala published successfully!") : t("Gaushala unpublished."));
    ...
    <div className="space-y-6 max-w-4xl">
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-6">
        ...
        <div className="flex items-center justify-between p-4 bg-slate-50 rounded-lg border border-slate-200">
          <div>
            <h4 className="font-medium text-slate-900">{t("Publish Gaushala")}</h4>
            <p className="text-sm text-slate-500 mt-1">{t("Toggle this to make your Gaushala visible to users for booking.")}</p>
          </div>
          <button
            onClick={handleTogglePublish}
            disabled={saving}
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none disabled:opacity-50 ${selectedOrg?.gaushalaPublished ? 'bg-purple-600' : 'bg-slate-300'}`}
          >
