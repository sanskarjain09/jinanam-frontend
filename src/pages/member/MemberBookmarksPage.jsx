...
export default function MemberBookmarksPage() {
  ...
  const {
    ...
    error,
    reload,
  } = useMemberList("/feed?savedOnly=true", { map: mapPost });
  const [posts, setPosts] = useState([]);
  useEffect(() => {
    setPosts(fetchedPosts);
  }, [fetchedPosts]);
  // Removed unused selectCategory code
  ...
  const interleaved = filtered; // Bookmarks don't need ad interleaving logic or we just skip it
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white rounded-3xl p-6 border border-slate-200/80 shadow-xs">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium mt-1">
            Posts and updates you have saved for later.
          </p>
        </div>
